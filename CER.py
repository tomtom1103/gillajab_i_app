...
def CER(ground_truth, users):
    ...
    for i, phon in enumerate(usr):
        append_md(phon, "white" if dif[i] else "red")
    print_md()


    append_md(Romanizer(ans).romanize(), "white")
    print_md()

    for i, val in enumerate(usr):
        append_md(Romanizer(val).romanize(), "white" if dif[i] else "red")
    print_md()

    dif2 = levenshtein(users, ground_truth)
    append_md(ground_truth + " ", "white")
    print_md()

    error_count = dict()

    for i, phon in enumerate(users):
        append_md(phon, "white" if dif2[i] else "red")
    print_md()
